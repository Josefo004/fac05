...
numeroALetras(xx:number) {
    let xxs = xx.toFixed(2);
    let num = parseFloat(xxs);
    let data = {
        numero: num,
        enteros: Math.floor(num),
        centavos: (((Math.round(num * 100)) - (Math.floor(num) * 100))),
        letrasCentavos: '',
        letrasMonedaPlural: 'Bs.',
        letrasMonedaSingular: 'Bs.',
        letrasMonedaCentavoPlural: 'ctvs.',
        letrasMonedaCentavoSingular: 'ctvs'
    };
    ...
    if (data.centavos > 0 && data.centavos < 10) {
      data.letrasCentavos = ' 0' + data.centavos +' '+ data.letrasMonedaCentavoPlural;
    } else if (data.centavos > 9) {
      data.letrasCentavos = ' ' + data.centavos +' '+ data.letrasMonedaCentavoPlural;
    } else {
      data.letrasCentavos = ' 00 '+data.letrasMonedaCentavoPlural;
    }
